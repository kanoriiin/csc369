...
void my_exit_group(int status)
{
    ...
    // lock to prevent other processes to make any modifications while this function is accessing the resources
    spin_lock(&my_table_lock);
    // get the pid of the exiting process and remove it
    del_pid(current->pid);
    // call the original system call
    // unlock to allow other processes to modify again
...
asmlinkage long interceptor(struct pt_regs reg) {
    ...
    // each intercepted system call will reach this function
    // if all pid of this syscall is monitored (meaning this is as well)
    ...
    // if only some of the pid of this system call is being monitored
    } else if (table[reg.ax].monitored == 1) {
        // if the pid of the current syscall is monitored
        if (check_pid_monitored(reg.ax, current->pid) == 1) {;
            // print information to log with macro log_message
            log_message(current->pid, reg.ax, reg.bx, reg.cx, reg.dx, reg.si, reg.di, reg.bp);
            spin_unlock(&my_table_lock);
            spin_unlock(&sys_call_table_lock);
            return table[reg.ax].f(reg);
...
// gate keeping helper function for my_syscall
// REQUEST_SYSCALL_INTERCEPT checker
 
long intercept(int syscall){
 
    int intercept_reading;
     
    // check to make sure the syscall is valid
    // (not negative, not > NR_syscalls-1, and not MY_CUSTOM_SYSCALL itself)
    if ((syscall < 0) || (syscall > NR_syscalls-1) || (syscall == MY_CUSTOM_SYSCALL)) {
        return -EINVAL;
    }
    // Check that the caller has the right permissions (root for this function)
    if (current_uid() != 0) {
        // if REQUEST_SYSCALL_INTERCEPT or REQUEST_SYSCALL_RELEASE or pid == 0 or (pid not own by calling process)
        return -EPERM;
        // -EPERM
    }
     
    // lock table before reading
    spin_lock(&my_table_lock);
    intercept_reading = table[syscall].intercepted;
    spin_unlock(&my_table_lock);
     
    // Check for -EBUSY (If intercepting a system call that is already intercepted)
    if (intercept_reading == 1) {
        return -EBUSY;
        // -EBUSY
    }
    ...
    // add the syscall to table, no longer needed as stated on piazza post 159
    // table[syscall].f = sys_call_table[syscall];
    // Update the table[syscall] as intercepted.
...
// REQUEST_SYSCALL_RELEASE checker
long release(int syscall){
    ...
    // Check for -EBUSY (If de-intercepting a system call that is not intercepted)
    if (intercept_reading == 0) {
        return -EBUSY;
        // -EBUSY
    }
    ...
    // Update the table[syscall] as not intercepted.
    table[syscall].intercepted = 0; 
    // Remove all the monitored pids.
...
// REQUEST_START_MONITORING checker
long startMonitoring(int syscall, int pid){
    int ownership;
    int insert;
    int monitor_reading;
 
    // if syscal !(not negative, not > NR_syscalls-1, and not MY_CUSTOM_SYSCALL itself)
    if ((syscall < 0) || (syscall > NR_syscalls-1) || (syscall == MY_CUSTOM_SYSCALL)) {
        return -EINVAL;
    }
 
    // When pid == 0 and we are not root user. 
    // The order of the error checking mattered here, as indicated by the comments above.
    if (pid == 0) {
        if (current_uid() != 0) {
            return -EPERM;
        }
    ...
    // if pid is invalid (-'ve pid) or (pid DNE, i.e. pid does not belong to a valid process)
    // if pid from a sysc that is not intercepted, it is not valid
    ...
    if (current_uid() != 0){
        ...
        // check if the owner of the pid is the right owner of the pid to be monitored
        ownership = check_pids_same_owner(current->pid, pid);
        if (ownership != 0) {
            return (long) ownership;
        }
 
    }
     
    // lock the table before any readings is done on it
    spin_lock(&my_table_lock);
    monitor_reading = table[syscall].monitored;
    spin_unlock(&my_table_lock);
 
 
    // CHANGE SYSCALL TO PID
    if ((check_pid_monitored(syscall, pid) == 1) || (monitor_reading == 2)) {
        // if (REQUEST_SYSCALL_INTERCEPT already existing system call) or (REQUEST_START_MONITORING already existing pid)
        return -EBUSY;
        // -EBUSY
    }
    ...
    if (pid == 0) {
        ...
        // When pid == 0, ALL processes are monitored.
        // destroy the monitored list and use it as a white list
        destroy_list(syscall);
        // Add pid to the monitored pid list.
        insert = add_pid_sysc(pid, syscall);
        // Therefore, monitored == 2. 
        table[syscall].monitored = 2;
    } else if (pid != 0) {
        // Add pid to the monitored pid list.
        insert = add_pid_sysc(pid, syscall);
        // Update the table[syscall] to 
        // some pids are monitored. monitored == 1.
...
// REQUEST_STOP_MONITORING checker
long stopMonitoring(int syscall, int pid){
    int ownership;
    int monitor_reading;
    ...
    // check if the pid is not monitored to begin with
    if (((check_pid_monitored(syscall, pid)) == 0) || (monitor_reading == 0)) {
        // -EINVAL
        return -EINVAL;
    }
    ...
    spin_lock(&my_table_lock);
    if (pid != 0) { 
        // this is -EINVAL if the syscall monitors all pid
        if (table[syscall].monitored == 2){
            spin_unlock(&my_table_lock);
            return -EINVAL;
        }
        // If pid != 0, some pid is monitored.
        // Therefore, delete the pid from the monitored list.
...
asmlinkage long my_syscall(int cmd, int syscall, int pid) {
    ...
    if (cmd == REQUEST_SYSCALL_INTERCEPT){
        ...
        return startMonitoring(syscall, pid);
    } else if (cmd == REQUEST_STOP_MONITORING) {
        return stopMonitoring(syscall, pid);
    }
...
static int init_function(void) {
    ...
    // allow rw of system_call_table
    set_addr_rw((unsigned long) sys_call_table);
     
     
    // using the syscall table size, add every syscall to table
    for(s = 0; s < NR_syscalls; s++) {
        // initialize the struct of the system call
        INIT_LIST_HEAD(&(table[s].my_list));
         
        // only when intercept, we assume that it would be slightly more efficient to have
        // system calls saved when we intercept them
        table[s].f = sys_call_table[s];
         
        // initial stats of if this system call is intercepted should be 0
        table[s].intercepted = 0;
        // initial value for if any pid is being monitored should be 0
        table[s].monitored = 0;
        // initial value of number of PIDs being monitored for this syscall should be 0
        table[s].listcount = 0;
    }
     
    // save original MY_CUSTOM_SYSCALL to orig_custom_syscall
    orig_custom_syscall = sys_call_table[MY_CUSTOM_SYSCALL];
    // get the exit group
    orig_exit_group = sys_call_table[__NR_exit_group];
 
    // hijack the original custom syscall with mysyscall
...
static void exit_function(void)
{
    ...
    // clear monitored pid of every syscall that is intercepted
     
    for (s = 0; s < NR_syscalls; s++){
        destroy_list(s);
    }
     
    // restore the original custom syscall
    sys_call_table[MY_CUSTOM_SYSCALL] = orig_custom_syscall;
    sys_call_table[__NR_exit_group] = orig_exit_group;
     
    // unlock to allow other processes to modify
    set_addr_ro((unsigned long) sys_call_table);
    spin_unlock(&my_table_lock);
    spin_unlock(&sys_call_table_lock);
